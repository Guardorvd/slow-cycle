...
4. **`BicycleController` is a stable API.** Other systems observe bicycle position/speed via properties and signals; they do not dictate internal kinematics. Internal banking, steering damping, and resistance curves may be refined within approved tasks (e.g. FEAT-006.2) while strictly preserving public signals, properties, and pitch raycasting.
...
6. **Mathematical continuity first.** The road generator must guarantee $C^1$ continuity as an invariant ($C^2$ is desirable where applicable). Do not rely on wheel raycasts to hide geometric tears, normal flips, or height steps.
