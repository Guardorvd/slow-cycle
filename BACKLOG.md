...
## 🎧 Следующий спринт: Спринт 3B (Геймпад, Звуковой Ландшафт и Камера ★ КЛЮЧЕВОЙ ДЛЯ АТМОСФЕРЫ)

### TASK: [FEAT-007.1a] Gamepad Analog Input Mapping
**Goal**: Поддержка геймпадов (Xbox, 8BitDo, DualShock) с плавными аналоговыми курками и стиками.
**Do**: Добавить оси геймпада в `project.godot`: RT (газ), LT (тормоз), Left Stick X (руль). В `bicycle_controller.gd`: чтение аналоговой силы нажатия курков (`Input.get_action_strength`), нелинейная кривая стика `pow(abs(stick), 1.5) * sign(stick)`, мёртвая зона 0.15. Назначить кнопки A (звонок), B (recovery), Y (смена камеры).
**Do not**: Не менять и не ломать раскладку клавиатуры (`W/A/S/D`, пробел, `R`, `V`).
**Acceptance Criteria**:
- При плавном нажатии RT разгон нарастает пропорционально ходу курка.
- При плавном нажатии LT торможение мягко дозируется.
- Левый стик с кривой 1.5 даёт ювелирный микро-контроль по центру и полный угол при упоре.
- Кнопки A/B/Y работают аналогично клавишам клавиатуры.
**Tests**: Тест в рантайме с контроллером; проверка fallback на клавиатуру без предупреждений движка.
**Files**: `project.godot`, `scripts/player/bicycle_controller.gd`.

### TASK: [FEAT-007.1b] Gamepad Haptic Vibration Feedback
**Goal**: Тактильный отклик через вибромоторы геймпада при выезде на траву и резком торможении.
**Do**: В `bicycle_controller.gd` вызывать `Input.start_joy_vibration(0, weak_magnitude, strong_magnitude, duration)`:
- Съезд на траву: короткий импульс (0.15с, слабая вибрация 0.3).
- Езда по траве: постоянная фоновая лёгкая микро-вибрация (0.1) пропорционально скорости.
- Экстренное торможение (`brake_input > 0.8` на скорости > 20 км/ч): ощутимый упругий толчок (0.25с, сильная вибрация 0.4).
**Do not**: Не спамить вызовы вибрации каждый кадр; запускать по таймеру или смене состояния.
**Acceptance Criteria**:
- Выезд на траву дает четкий тактильный клик.
- На ровной дороге вибрации нет.
- При отключенном геймпаде вызовы не вызывают ошибок.
**Tests**: Запуск без геймпада (zero errors) и с подключенным геймпадом.
**Files**: `scripts/player/bicycle_controller.gd`.

### TASK: [FEAT-007.2] Процедурный звуковой ландшафт (Ветер + Шуршание гравия)
**Goal**: Живой процедурный звук движения на скорости и шуршания покрышек по разным поверхностям без внешних тяжелых .wav файлов.
**Do**: В `bike_audio_manager.gd`:
- Настроить `wind_player`: непрерывный синтез розового шума с полосовым фильтром BandPass (200–1800 Гц), частота среза и громкость плавно нарастают от скорости от 15 до 45 км/ч.
- Добавить процедурный `gravel_player`: зернистый шум шуршания покрышек, громкость от скорости, при выезде на траву (`is_on_grass`) частотный спектр смещается в глухой низкий бас (глушение травой).
**Do not**: Не использовать внешние аудиофайлы; сохранять полностью процедурную архитектуру генерации через `AudioStreamGenerator` или `AudioStreamWAV`.
**Acceptance Criteria**:
- На спуске со скоростью 35+ км/ч слышен мягкий, кинематографичный свист ветра в ушах.
- На гравии слышно приятное потрескивание мелких камней под колесом.
- При остановке ($v \to 0$) звук плавно затихает до полной тишины.
**Tests**: Проверка уровня громкости в дБ, отсутствие клиппинга и щелчков звукового буфера.
**Files**: `scripts/audio/bike_audio_manager.gd`.

### TASK: [FEAT-007.3] Dynamic Speed FOV
**Goal**: Подсознательное усиление ощущения скорости через динамическое расширение поля зрения камеры.
**Do**: В `bike_camera.gd`:
- Плавно расширять FOV первой камеры от базовых 78° до 83° при разгоне от 20 до 45 км/ч.
- Для третьей камеры: расширение от 68° до 72°.
- Использовать мягкий `lerpf` со скоростью сглаживания 4.0, исключая рывки при быстром торможении.
**Do not**: Не превышать 84°, чтобы избежать искажения "рыбий глаз" (fisheye distortion).
**Acceptance Criteria**:
- При разгоне под горку до 40 км/ч мир плавно слегка отдаляется, усиливая чувство полета.
- При торможении угол обзора плавно возвращается в исходное состояние за 0.8–1.0 сек.
**Tests**: Автоматический тест в `test_diagnostics.gd` на границы FOV [78.0°, 83.0°].
**Files**: `scripts/camera/bike_camera.gd`.

---

## 🌅 Будущий спринт: Спринт 4A (Атмосфера, Суточный Цикл и Фара)
