...
## 🚴 Следующий спринт: Спринт 4B (Руление, динамика наклона и траектории виражей)

### TASK: [FEAT-012.2] Sprint 4B: Руление, динамика наклона и траектории виражей (Steering, Lean Dynamics & Apex Flow)
**Goal**: Велосипед входит в поворот через наклон рамы с ощущением выбора траектории, центробежной силой и сбросом скорости при слишком агрессивном угле (cornering scrub).
**Do**:
- В `scripts/player/bicycle_controller.gd`:
  - Построить цепочку: $Input \rightarrow Desired Lean \rightarrow Lean Dynamics \rightarrow Steering Response \rightarrow Lateral Accel \rightarrow Yaw$.
  - Динамический лимит угла руления в зависимости от скорости (speed-sensitive steering) с сохранением безопасного радиуса виража ($R \ge 25.5\text{м}$).
  - Упругий возврат руля и рамы в нейтральное положение (centering moment) при отпускании управления.
  - Потеря скорости при чрезмерно резком закладывании виража (cornering scrub) — стимулирует игрока притормаживать перед поворотом и грамотно выбирать траекторию.
**Do not**: Не допускать мгновенного поворота модели вокруг вертикальной оси без наклона; исключить раскачку и паразитные колебания.
**Acceptance Criteria**:
- На малых скоростях руль поворачивает отзывчиво; на высокой скорости поворот происходит через глубокий наклон рамы.
- Резкий вход в поворот на высокой скорости без торможения приводит к понятному сбросу скорости от боковой нагрузки.
- Выход из виража плавный и центрированный.
**Tests**: Тесты прохождения пологой дуги, острого поворота под 90° и S-образной змейки.
**Files**: `scripts/player/bicycle_controller.gd`.

---

### TASK: [FEAT-012.3] Sprint 4C: Реакция на рельеф и типы поверхностей (Terrain, Crest/Dip & Surface Dynamics)
...
**Do**:
- В `scripts/player/bicycle_controller.gd`:
  - Расширить модель поверхностей (`GRAVEL`, `GRASS`, `ROUGH_GRAVEL`).
  - Ввести канал шероховатости `terrain_roughness` (0.0..1.0).
  - Сгладить контакт переднего и заднего лучей (`front_ray`, `rear_ray`) на резких гребнях (Crest) и ямах (Dip), исключая подпрыгивания, провалы и удары тангажа.
**Do not**: Не менять генерацию геометрии полотна дороги в `road_chunk.gd`.
**Acceptance Criteria**:
- Велосипед плотно следует рельефу на острых гребнях и во впадинах без зависания в воздухе.
- Выезд на траву дает четко ощутимое сопротивление качению и бархатистую мягкость.
**Tests**: Тест огибания искусственного гребня и впадины.
**Files**: `scripts/player/bicycle_controller.gd`.

---

### TASK: [FEAT-012.4] Sprint 4D: Слой визуального представления велосипеда (Bicycle Visual Presentation Layer)
**Goal**: Чистое разделение физического движения капсулы и выразительного визуального отображения рамы, вилки, руля и спиц колес.
**Do**:
- В `scripts/player/bicycle_controller.gd` и `scenes/player/bicycle.tscn`:
  - Синхронизировать визуал руля, компенсацию оси передней втулки (`FrontAxle`) и вращение спиц колес от скорости движения при свободном ходе и педалировании.
  - Наклон рамы и клевок при торможении применять исключительно к визуальной ноде (`VisualsRoot`).
**Do not**: Не менять физический корень `CharacterBody3D`.
**Acceptance Criteria**:
- Руль выразительно поворачивает в руках от 1-го лица, спицы крутятся пропорционально пройденному пути при свободном накате, рама наклоняется синхронно виражу.
**Files**: `scenes/player/bicycle.tscn`, `scripts/player/bicycle_controller.gd`.
